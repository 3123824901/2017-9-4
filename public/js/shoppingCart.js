$(function() {
	...
	var obj=localStorage.getItem('localData')? JSON.parse(localStorage.getItem('localData')):{};	
	var sum = 0;
	for(var i in obj) {
		//console.log(obj[i]);
		sum += obj[i];
	...
	//保存COOKIE
	function setStorage(id,nums){
		var localData=localStorage.getItem('localData')?JSON.parse(localStorage.getItem('localData')):{};
		//同种商品加数量，不同商品添属性
		if(localData[id]==undefined){
            localData[id] = nums ;
        }else{
        	localData[id]= nums+localData[id];
        }		
	...
	//移除cookie
	function remoStorage(proId){
		var localData=JSON.parse(localStorage.getItem('localData'));
		delete localData[proId];//对象删除某个属性和对应的值
		console.log(localData,localData[proId],localData)
		localStorage.setItem("localData",JSON.stringify(localData));		
	}
